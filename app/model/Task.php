...
function insert_task($conn, $data){
	$sql = "INSERT INTO tasks (title, description, assigned_to, due_date, file_name) VALUES(?,?,?,?,?)";
...
function count_tasks_NoDeadline($conn){
	$sql = "SELECT id FROM tasks WHERE (due_date IS NULL OR due_date = '0000-00-00' OR due_date = '') AND status != 'Completed'";
	$stmt = $conn->prepare($sql);
	$stmt->execute([]);
...
function update_task($conn, $data){
	$sql = "UPDATE tasks SET title=?, description=?, assigned_to=?, due_date=?, file_name=? WHERE id=?";
	$stmt = $conn->prepare($sql);
	$stmt->execute($data);
}
...
function count_my_tasks_NoDeadline($conn, $id){
	$sql = "SELECT id FROM tasks WHERE assigned_to=? AND (due_date IS NULL OR due_date = '0000-00-00' OR due_date = '') AND status != 'Completed'";
	$stmt = $conn->prepare($sql);
	$stmt->execute([$id]);
...
function search_tasks($conn, $search_term){
	$sql = "SELECT t.*, u.full_name as assigned_user_name 
			FROM tasks t 
